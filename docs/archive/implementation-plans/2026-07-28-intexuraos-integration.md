...
- Tasks 1–6 modify only IntexuraOS. Task 7 adds only the explicitly listed
  acceptance tooling and runbook to SentryBox.
...
- SentryBox webhook bodies remain valid current `event_alert.triggered` payloads; the existing route, HMAC header, raw-body verification, and parser shape remain compatible.
- SentryBox webhook transition identity comes from the already parsed event ID. Historical Sentry payloads without an event ID keep the legacy fallback.
- SentryBox human and worker reads remain private through Tailscale. The fixed MCP compatibility token is syntactic input for the official client, not authentication.
...
- [ ] Build SentryBox transition keys from organization, project identity, stable issue ID, and event ID. For historical payloads without event ID, retain the current resource/action suffix as a compatibility fallback.
...
- [ ] Preserve the current blocking rule for active tasks and open pull requests. Treat merged/closed PRs, archived tasks, deleted linked tasks, and terminal failures as non-blocking for a later SentryBox event ID.
...
- [ ] Assert the current parser produces organization, project ID/slug, issue ID, URL, status, title, and event ID without a SentryBox-specific body field.
...
- [ ] Run the three focused test files; expect the SentryBox fixture to traverse the real verifier and parser.
- [ ] Commit with `test(code-agent): lock sentrybox webhook compatibility`.

## Task 3: Pin Sentry MCP and add the SentryBox server entry
...
- [ ] Read optional startup configuration `INTEXURAOS_ERROR_HUB_HOST`. Reject schemes, paths, credentials, and whitespace; accept a DNS host with optional port. The cutover gate, rather than historical-only startup, requires this value before SentryBox webhooks can be enabled.
...
- [ ] Add tests proving a missing SentryBox host does not break historical-only operation, an invalid supplied host fails startup clearly, and the exact valid host reaches the container environment.
- [ ] Build the Code Worker image and run `sentry-mcp --version`; expect `0.37.0` and no network installation at task start.
- [ ] Commit with `feat(orchestrator): add pinned sentrybox mcp target`.
...
- [ ] Write failing prompt tests for a `sentry.io` historical SaaS URL and the
  configured private SentryBox permalink host.
...
- [ ] Define fallback reads as the same five private Sentry-compatible REST routes. For SentryBox URLs, the fixed syntactic bearer string carries no authority; network reachability is the access boundary.
...
- [ ] Commit with `feat(orchestrator): route sentry tasks to sentrybox evidence`.

## Task 5: Prove a real Code Worker can read the private SentryBox
...
- [ ] Add a failing container test against a disposable local HTTPS-compatible SentryBox fixture that invokes `get_issue_details` and `search_issue_events` through the `error_hub` MCP entry.
...
- [ ] On Home Dev staging, seed a non-production SentryBox issue and run the script from one disposable Code Worker container. Expect issue title, project, environment, release, stack, and event ID to match.
- [ ] If the worker bridge cannot route to the host Tailscale address, add only the documented host-gateway route in deployment configuration; never attach the SentryBox private listener to `code-worker-net`.
- [ ] Record the successful command and sanitized output in the deployment evidence; do not enable Code Agent SentryBox webhooks yet.
- [ ] Commit with `test(orchestrator): verify private sentrybox mcp access`.
...
- [ ] Start a local envelope-capture fixture and write a failing test that initializes the current Node SDK with a SentryBox-shaped DSN.
...
- [ ] Do not add a SentryBox event ID to Pino logs in this migration; correlation uses existing request/trace/task identifiers and the documented time/message fallback.
- [ ] Run `pnpm --filter @intexuraos/infra-sentry test && pnpm --filter @intexuraos/web test`; expect DSN-only compatibility.
- [ ] Commit with `test(sentry): prove sentrybox dsn compatibility`.

## Task 7: Validate the full automation flow before cutover

**Repositories:** `intexuraos-2` and `sentrybox`

**Files:**

- Create in SentryBox: `scripts/acceptance/emit-controlled-issue.mjs`
- Create in SentryBox: `scripts/acceptance/verify-code-agent-flow.mjs`
- Create in SentryBox: `docs/runbooks/automation-acceptance.md`
...
- [ ] Verify SentryBox storage shows one unresolved issue, the correct project/version/environment, one occurrence, one created outbox row, and a successful HMAC delivery.
...
- [ ] Retry the same SentryBox delivery and assert there is no second Linear issue/task.
- [ ] Complete or close the controlled task, resolve the SentryBox issue, emit a new event ID, and verify exactly one later task can be created for the regression.
- [ ] Verify the real worker reads issue evidence from SentryBox and still completes the unchanged `SENTRY_AGENT_FINAL` contract.
- [ ] Keep all production webhook routes disabled throughout this test.
- [ ] Commit SentryBox acceptance tooling with `test: add code agent automation acceptance flow`.
...
- None in IntexuraOS. The worker consumes SentryBox's private Sentry-compatible
...
- [ ] Confirm one SentryBox issue retry and one SentryBox regression have the expected one-task-per-transition behavior.
- [ ] Do not enable production SentryBox webhooks until the Home Dev deployment/cutover plan reaches its explicit production gate.
