...
This runbook proves the development-only Error Hub automation path before any
...
```text
backend-dev DSN -> Error Hub ingest/grouping/outbox -> signed Code Agent webhook
  -> one Linear issue -> one agentType=sentry Code Task -> private Hub evidence
```

Production Error Hub webhook destinations and production Sentry webhook
sources remain unchanged throughout this procedure.

## Preconditions

1. Error Hub readiness is healthy on the private Home Dev Tailscale origin.
2. The backend development DSN is available only as `ERROR_HUB_DEV_DSN`. Do not
   ...
   `environment=dev`, project ID `1`, and the public Hub hostname. A production
   key is rejected by the Hub's environment binding and makes the command fail.
...
read -r -s -p "backend-dev Hub DSN: " ERROR_HUB_DEV_DSN
...
- one unresolved `intexuraos-backend` issue in `dev` at release
  `intexuraos-error-hub-acceptance@1.0.0`;
...
- exactly one current Code Task containing the exact private Hub issue URL,
...
and must not attach Error Hub to `code-worker-net`.
...
This requires a successful terminal task, a received callback, the exact Hub
...
## 4. Close the first transition and resolve the Hub issue

Keep the `taskId` and `linearIssueId` from step 1 as non-secret evidence. The
close phase deletes only the single task selected by the exact controlled Hub
URL and resolves only the corresponding Hub issue. It refuses the mutation
...
5. Preserve only sanitized timestamps and the Hub/Code Task/Linear IDs. Never
   preserve DSNs, HMACs, access tokens, webhook bodies, or credential hashes.

Failure of any assertion stops the cutover. Do not enable production Error Hub
