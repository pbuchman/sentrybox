...
**Goal:** Deploy SentryBox safely on Home Dev, expose only Sentry ingest and a tightly scoped deployment callback publicly, expose operator/worker reads only through Tailscale, and migrate IntexuraOS from SaaS Sentry with measured shadow phases and immediate rollback.

**Architecture:** GitHub-hosted Actions build and publish an immutable public GHCR image. Home Dev runs one hardened container under systemd with two loopback listeners and persistent SQLite storage. Cloudflare Tunnel plus Caddy expose the write-only Envelope endpoint and a separate signed deployment handler; Tailscale Serve exposes the private UI/API. Environment-bound DSNs are switched dev first, then production, while SentryBox forwards envelopes to legacy Sentry during comparison windows.
...
- SentryBox repository owns `deploy/home-dev/*`; `pbuchman-dev` only documents installation and references those canonical files.
...
- Keep 30-day event retention and the 5 GiB live-data budget fixed; do not add
  environment, manifest, or command-line overrides for either limit.
- Keep SentryBox Code Agent destinations in `disabled` mode during shadow comparison;
...
services:
  sentrybox:
    ...
    volumes:
      - /home/pbuchman/services/sentrybox/data:/data
      - /home/pbuchman/services/sentrybox/env:/run/secrets/error-hub-env:ro
```
...
- [ ] Commit with `build: package hardened sentrybox container`.
...
- Create: `deploy/home-dev/sentrybox.service`
- Create: `deploy/home-dev/sentrybox-deploy.service`
- Create: `deploy/home-dev/sentrybox-backup.service`
- Create: `deploy/home-dev/sentrybox-backup.timer`
- Create: `deploy/home-dev/sentrybox-restore-test.service`
- Create: `deploy/home-dev/sentrybox-restore-test.timer`
...
```text
/home/pbuchman/deploy/sentrybox
/home/pbuchman/services/sentrybox/env
/home/pbuchman/services/sentrybox/data
/home/pbuchman/services/sentrybox/backups
```
...
**Repositories:** `sentrybox` and `pbuchman-dev`

**Files in SentryBox:**

- Create: `deploy/home-dev/caddy-sentrybox.caddy`
- Create: `deploy/home-dev/caddy-sentrybox-deploy.caddy`
...
- [ ] Add a separate Caddy vhost exposing only `POST errors-deploy.intexuraos.cloud/github/workflow-run` to `127.0.0.1:9003`; the SentryBox application and deploy handler remain separate processes.
...
- [ ] Confirm a disposable real Code Worker can reach private HTTPS port 8443 without attaching SentryBox to `code-worker-net`; add only a narrow host-gateway route if the test proves it is needed.
- [ ] Update `pbuchman-dev` documentation to point at SentryBox's checked-in deployment assets as canonical; do not copy/fork those files.
- [ ] Commit SentryBox changes with `ops: define public and private network boundaries`; commit the documentation-only `pbuchman-dev` change separately.

## Task 6: Rotate Cloudflare credentials and install the deployment webhook

**Repositories:** `sentrybox` and Home Dev runtime configuration

**Files:**

- Create: `deploy/home-dev/deploy-webhook.mjs`
- Create: `deploy/home-dev/deploy-webhook.test.mjs`
- Create: `deploy/home-dev/sentrybox-deploy-webhook.service`
...
**Repositories:** `sentrybox`, `intexuraos-2`, and Home Dev runtime configuration

**Files:**

- Create in SentryBox: `docs/runbooks/dev-shadow-cutover.md`
- Create in SentryBox: `scripts/acceptance/compare-shadow-events.mjs`

- [ ] Record current dev backend/web DSNs by secret reference only, current Sentry webhook configuration, deployed IntexuraOS SHA, SentryBox image digest, and rollback owner/time window.
- [ ] Deploy SentryBox with Code Agent destinations in `disabled` mode and dev legacy-forwarding enabled; verify new shadow transitions are recorded only as non-dispatchable `suppressed` rows.
- [ ] Change only `INTEXURAOS_SENTRY_DSN` and `INTEXURAOS_SENTRY_DSN_WEB` in `/home/pbuchman/deploy/intexuraos/.envrc.local` to the environment-bound SentryBox dev DSNs.
...
- [ ] Compare event IDs and supported fields between SentryBox and shadow-forwarded Sentry for at least 48 hours. Record accepted, discarded, forwarded, failed, grouped, and storage-growth counts.
- [ ] Stop if any supported event reaches Sentry but not SentryBox, telemetry changes an application response, environment/key mismatch is accepted, redaction fails, or projected storage exceeds the budget.
- [ ] After the Code Agent lease fix and real-worker MCP test are deployed, disable only dev Sentry webhook sources and enable only SentryBox dev destinations. Verify suppressed shadow rows remain non-dispatchable and the pending queue is empty at the baseline.
- [ ] Run one controlled creation and one resolved regression; verify one Linear issue and Code Task per transition, correct worker type, SentryBox evidence read, unchanged completion contract, and successful outbox state.
...
**Repositories:** `sentrybox`, `intexuraos-2`, and production configuration

**Files:**

- Create in SentryBox: `docs/runbooks/production-cutover.md`
- Create in SentryBox: `docs/runbooks/rollback.md`
- Create in SentryBox: `scripts/acceptance/cutover-gate.mjs`

- [ ] Require signed evidence that every development acceptance item passed, no unresolved dead letters exist, backup/restore checks pass, 5 GiB forecast is safe, and a rollback drill has restored a prior SentryBox digest.
- [ ] Configure production legacy forwarding and Code Agent destination references, but leave SentryBox production destinations in `disabled` mode and verify suppression semantics before traffic.
- [ ] Replace the production backend and web DSN values in the existing Secret Manager entries with the two environment-bound SentryBox production DSNs; do not alter application call sites.
...
- [ ] Emit controlled production backend and browser events and compare SentryBox with shadow-forwarded Sentry for at least 48 hours across project, release, environment, service, level, event ID, payload evidence, and volume.
...
- [ ] Disable the two migrated production Sentry webhook sources, then enable the matching SentryBox production destinations. Never leave both automation sources active.
- [ ] Run one controlled production creation and regression, then immediately disable the controlled alert. Verify exact one-task behavior and record SentryBox/Linear/Code Task/PR evidence.
- [ ] Perform a timed rollback drill: disable SentryBox webhooks first, re-enable Sentry webhooks, restore old DSN secret versions, run the normal backend/web/function deployment, and confirm new telemetry reaches Sentry. Then return to SentryBox only after review.
...
**Repositories:** `sentrybox` and `intexuraos-2`

**Files:**

- Create in SentryBox: `docs/runbooks/sentry-retirement.md`
...
- [ ] Disable SentryBox-to-Sentry forwarding and verify SentryBox counts remain stable for 24 hours.
...
- [ ] Do not import old Sentry history into SentryBox; historical links remain historical and SentryBox retention starts at cutover ingestion.
...
- Home Dev private Tailscale Serve gains HTTPS port 8443 for SentryBox UI/read
  traffic.
...
- After stable cutover only: active SaaS Sentry webhook sources and direct DSN
  reporting to SaaS Sentry. DSN reporting to SentryBox remains the primary
...
- All pre-existing Home Dev public host routes other than the two explicitly
  added SentryBox hostnames.

## Completion gate

- [ ] Record immutable SentryBox and IntexuraOS SHAs/digests, project/environment key mapping, public/private route probes, backup/restore result, and all dev/prod acceptance evidence without secret values.
...
- [ ] Confirm rollback can restore Sentry reporting and the previous SentryBox image without database reversal.
