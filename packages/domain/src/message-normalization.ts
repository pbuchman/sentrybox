...
const HASH =
  /(?<![0-9a-z])(?:[0-9a-f]{64}|[0-9a-f]{40}|[0-9a-f]{32})(?![0-9a-z])/gi;
const ISO_TIMESTAMP =
  /\b\d{4}-\d{2}-\d{2}[t ]\d{2}:\d{2}:\d{2}(?:\.\d{1,9})?(?:z|[+-]\d{2}:?\d{2})\b/gi;
const UNIX_TIMESTAMP = /(?<![0-9a-z])\d{10}(?:\d{3})?(?![0-9a-z])/gi;
...
const STANDALONE_NUMERIC_IDENTIFIER = /(?<![0-9a-z.])\d{4,}(?![0-9a-z.])/gi;
...
export function normalizeMessageTemplate(value: string): string {
  return value
    ...
    .replace(STANDALONE_NUMERIC_IDENTIFIER, "{number}");
}
